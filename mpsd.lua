...
function prepare_sources() 
	...
	for _,val in ipairs(talimat.kaynak) do
		...
		place="url"
		if key == "git" or key == "github"	then place="git" end
		if key == "dosya" or key == "file"	then place="file" end
		
		if  place == "url"  then store=rules.dirs.archive..store end
		if  place == "file" then 
			address=talimat_dir.."/"..address 
			store=rules.dirs.src.."/"..store
		end 
	    -- add sources table
		table.insert(t,{fetch=place,address=address,store=store})
	end
	return t
...
function extract_sources()
	...
	local aext, suffix
	local archive_suffix={"gz","xz","lz","bz2"}
